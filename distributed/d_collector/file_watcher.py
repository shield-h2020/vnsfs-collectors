...
class FileWatcher(Observer):
    ...
    :param collector_path : Directory path that will be monitored.
    ...
    def __init__(self, collector_path, supported_files, recursive):
        self._logger  = logging.getLogger('SHIELD.DC.FILE_WATCHER')
        self._queue   = []

        super(FileWatcher, self).__init__()

        if isinstance(recursive, basestring):
            recursive = recursive.lower().strip() == 'true'

        self._logger.info('Schedule watching "{0}" directory.'.format(collector_path))
        super(FileWatcher, self).schedule(NewFileEventHandler(self), collector_path, recursive)
