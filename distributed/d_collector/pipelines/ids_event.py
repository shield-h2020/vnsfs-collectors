'''
    Methods that will be used to process and prepare ids event data, before being sent to
Kafka cluster.
'''
...
def convert(jsonfile, tmpdir, opts='', prefix=None):
    '''
        Convert JSON file to a comma-separated output format.

    :param jsonfile: Path of the JSON data file.
    :param tmpdir  : Path of local staging area.
    :param opts    : A set of options; no use here.
    :param prefix  : If `prefix` is specified, the file name will begin with that;
                     otherwise, a default `prefix` is used.
    :returns       : Path of CSV-converted file.
    :rtype         : ``str``
    '''
    logger = logging.getLogger('SHIELD.DC.PROCESS.IDS_EVENT')

    def getter(key, event, default=''):
        if key in event and event[key] != 'null':
            return str(event[key])
        return default

    with tempfile.NamedTemporaryFile(prefix=prefix, dir=tmpdir, delete=False) as fp:
        # .............................read JSON file
        for event in [x.values()[0] for x in json.loads(open(jsonfile).read())]:
            unix_tstamp = '%d.%d' % (event['event-second'], event['event-microsecond'])

            # .........................convert to CSV row
            fp.write('{0}\n'.format(','.join([
                datetime.utcfromtimestamp(float(unix_tstamp)).strftime('%Y%m%d %H%M%S'),
                getter('blocked', event),
                getter('classification', event),
                getter('classification-id', event),
                getter('destination-ip', event),
                getter('dport-icode', event),
                getter('event-id', event),
                getter('generator-id', event),
                getter('impact', event),
                getter('impact-flag', event),
                getter('priority', event),
                getter('protocol', event),
                getter('sensor-id', event),
                getter('signature-id', event),
                getter('signature-revision', event),
                getter('sport-itype', event),
                getter('source-ip', event),
                getter('vlan-id', event),
                unix_tstamp])))
        return fp.name
...
def prepare(csvfile, max_req_size):
    ...
    with open(csvfile, 'r') as fp:
        for line in fp:
            ...
            # .........................assume the first 11 characters of the `search`
            # result as the `partition`, e.g. '20180320 09'
            if match.group()[:11] == partition and (msg_size + size) < max_req_size:
