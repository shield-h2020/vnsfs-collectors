'''
    Methods that will be used to process and prepare ids packet data, before being sent to
Kafka cluster.
'''
...
def convert(jsonfile, tmpdir, opts='', prefix=None):
    ...
    :rtype         : ``str``
    :raises OSError: If an error occurs while parsing the JSON file.
    '''
    logger = logging.getLogger('SHIELD.DC.PROCESS.IDS_PACKET')

    def getter(key, packet, default=''):
        if key in packet and packet[key] != 'null':
            return str(packet[key])
        return default

    with tempfile.NamedTemporaryFile(prefix=prefix, dir=tmpdir, delete=False) as fp:
        # .............................read JSON file
        for packet in [x.values()[0] for x in json.loads(open(jsonfile).read())]:
            unix_tstamp = '%d.%d' % (packet['packet-second'], packet['packet-microsecond'])

            # .........................convert to CSV row
            fp.write('{0}\n'.format(','.join([
                datetime.utcfromtimestamp(float(unix_tstamp)).strftime('%Y%m%d %H%M%S'),
                getter('data', packet),
                getter('event-id', packet),
                getter('event-second', packet),
                getter('length', packet),
                getter('linktype', packet),
                getter('sensor-id', packet),
                unix_tstamp])))
        return fp.name
